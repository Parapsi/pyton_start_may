...
class Order:
    ...
    def __iter__(self):
        for dish, price in self.your_order.items():
            yield dish, price

    def __getitem__(self, index):
        if 0 <= index < len(self.your_order):
            return self.your_order[index]
        else:
            return IndexError("Wrong index!")

    def add_item(self, item):
        self.your_order[self.index] = [item.name, item.price]
        self.index += 1
    ...
    def calculate_total(self, discount: Discount):
        ...
        for item in self.your_order:
            summ += self.your_order[item][1]
        res = ''
        for item in self.your_order:
            res += f'{self.your_order[item][0]} '
